...
local function export_state(command)
    ...
    if EXPORT_TO_FILE then
        ...
        for surface_name, surface_data in pairs(state.surfaces) do
            ...
            metadata.total_entities.assembling_machines = metadata.total_entities.assembling_machines + surface_summary.entity_counts.assembling_machines
            metadata.total_entities.mining_drills = metadata.total_entities.mining_drills + surface_summary.entity_counts.mining_drills
            ...
            if #surface_data.entities.assembling_machines > 0 then
                helpers.write_file(base_folder .. "assembling-machines/assembling_machines_" .. surface_name .. "_" .. timestamp .. ".json", 
        ...
        game.print("Total entities: " .. (metadata.total_entities.assembling_machines + metadata.total_entities.mining_drills + metadata.total_entities.generators + metadata.total_entities.labs))
    end
end

-- Automatic export every hour (server only)
script.on_nth_tick(EXPORT_INTERVAL_TICKS, function(event)
    -- Only run on server, not on clients
    if game.server or not game.is_multiplayer() then
        game.print("Automatic factory metrics export...")
        export_state(nil)
    end
end)
